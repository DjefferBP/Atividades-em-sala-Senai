...
int calculoNum(int a, int b)
{
    ...
    else if ((a % 2 == 0) && (b % 2 != 0) || (b % 2 == 0) && (a % 2 != 0))
    {
        if (a > b)
        {
            maior = a;
            menor = b;
            WriteLine($"Um dos números é ímpar e o outro par, portanto, devemos subtrair o maior do menor, resultando em:{maior} - {menor} = {maior - menor}.");
        }
	else if (a == b){
	    WriteLine($"Como os dois número são iguais, apenas devemos somá-los, resultando em: {maior} - {menor} =  {maior - menor}.");
        }
        else
        {
            maior = b;
...
num1 = Convert.ToInt32(ReadLine());
WriteLine("Digite outro número inteiro: ");
num2 = Convert.ToInt32(ReadLine());
calculoNum(num1, num2);
